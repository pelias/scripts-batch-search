...
function createSearchStream(queryParams) {
  ...
  return through.obj(function (data, enc, next) {
    if (!data.hasOwnProperty('lat')) {
      console.log(data);
      return next(new Error('no lat column'));
    }

    if (!data.hasOwnProperty('lon')) {
      console.log(data);
      return next(new Error('no lon column'));
    }
    ...
    this_query_params['point.lat'] = data.lat;
    this_query_params['point.lon'] = data.lon;
...
function addResData(data, resData) {
  data.geojson = Buffer.from(JSON.stringify(resData[0])).toString('base64');
